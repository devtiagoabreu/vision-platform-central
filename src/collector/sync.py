...
from src.storage.database import ObservationRecord, SessionLocal

logger = logging.getLogger(__name__)


def collect_from_local(client: LocalClient | None = None) -> dict:
    ...
    try:
        for obs in observations:
            ...
            record = ObservationRecord(
                ...
                image_uri=obs.get("image_uri", ""),
                sha256=obs.get("sha256", ""),
                width=obs.get("width", 0),
                height=obs.get("height", 0),
                quality_score=obs.get("quality", {}).get("score", 0),
